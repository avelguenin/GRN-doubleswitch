...
class GeneNetwork:
    ...
    def __init__(self,k,s,d,Theta):
        ...
        self.S = s ## production matrix
        self.D = d ## degradation matrix
    ...
    def computeSwitchRates(self,Delta):
        w = []
        p_act = self.Theta.dot(self.p)/34000 # 34000 is the max concentration at given parameters
        p_act = [30*x for x in p_act]
        print("p_act = " + str(p_act))
        for i in range(self.size) :
            if self.g[i] == 1:
                w.append(self.K[1])
            elif self.g[i] == 0:
                w.append(self.K[0] / (1 + 3*np.exp(- p_act[i] - Delta[i])))        
        print("w = " + str(w))
